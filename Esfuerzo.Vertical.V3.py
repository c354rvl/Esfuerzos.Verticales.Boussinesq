# -*- coding: utf-8 -*-
"""
Created on Thu Jan 22 07:59:58 2026
Código base generado por la IA Gemini 3 con la formulación de contorno 
planteada por el pseudocódigo de César Vázauez Lorenzana en el curso de 
Comportamiento de suelos, y modificado por el autor para las necesidades
de análisis y versatilidad de Python.
@author: César Vázquez lorenzana
"""

import numpy as np
from scipy.integrate import quad
import matplotlib.pyplot as plt
import os

def G_func(A, B, z):
    """Función de influencia base G(A, B, z)."""
    if z < 1e-6: z = 1e-6 # Evitar división por cero en la superficie
    den1 = 3 * (A**2 + z**2)**2 * np.sqrt(A**2 + B**2 + z**2)
    if den1 < 1e-12: return 0
    term1 = B / den1
    term2 = 2 + ((A**2 + z**2) / (A**2 + B**2 + z**2))
    return term1 * term2

def calcular_sigma_z(x_pts, y_pts, q_pts, x0, y0, z):
    """Implementa la sumatoria de esfuerzos según la ecuación (19)."""
    n = len(x_pts)-1
    # Traslado de vértices al punto de análisis P(xo,yo,z)
    x = x_pts-x0
    y = y_pts-y0
    total_sum = 0
    for i in range(n):
        dx_i = x[i+1] - x[i]
        dy_i = y[i+1] - y[i]
        qi = q_pts[i]
        # Integrando: G(Yt, Xt, z)dy - G(Xt, Yt, z)dx
        def integrando(t):
            xt = (x[i] + t * dx_i)
            yt = (y[i] + t * dy_i)
            return G_func(yt, xt, z) * dy_i - G_func(xt, yt, z) * dx_i
        integral_i, _ = quad(integrando, 0, 1)
        total_sum += qi * integral_i
    return (3 * (z**3) / (4 * np.pi)) * total_sum

def graficar_planta_xy(x_pts, y_pts, q_pts):
    z_fija = float(input("Ingrese profundidad de análisis (z): "))
    ext1 = float(input("Ingrese valor mín de extensión del plano (ej. -10): "))
    ext2 = float(input("Ingrese valor máx de extensión del plano (ej. 10): "))
    res = 40

    print(f"Calculando esfuerzos verticales en plano z={z_fija}...")    
    x_range = np.linspace(ext1, ext2, res)
    y_range = np.linspace(ext1, ext2, res)
    X, Y = np.meshgrid(x_range, y_range)
    Sigma = np.array([[calcular_sigma_z(x_pts, y_pts, q_pts, xi, yi, z_fija) for xi in x_range] for yi in y_range])
    
    # Graficando
    plt.figure(figsize=(9, 7))

    # Dibujar curvas de nivel rellenas
    cp = plt.contourf(X, Y, Sigma, levels=20, cmap='gist_rainbow_r')
    plt.colorbar(cp, label='Esfuerzo Vertical ($\\sigma_z$)')

    # Dibujar líneas de contorno
    lineas = plt.contour(X, Y, Sigma, levels=20, colors='black', linewidths=0.5)
    plt.clabel(lineas, inline=True, fontsize=10)    

    # Dibujar el polígono de carga para referencia
    plt.plot(x_pts, y_pts, 'k--', linewidth=2, label='Área de Carga')
    
    plt.title(f"Plano Horizontal XY (z = {z_fija})")
    plt.xlabel("x"); plt.ylabel("y"); plt.legend(); plt.grid(alpha=0.3); plt.show()

def graficar_perfil_xz(x_pts, y_pts, q_pts):
    y_fijo = float(input("Ingrese plano de corte Y (ej. y=2): "))
    ext_x1 = float(input("Ingrese valor horizontal mínimo Xmín (ej. -10): "))
    ext_x2 = float(input("Ingrese valor horizontal mmáximo Xmáx (ej. 10): "))
    z_max = float(input("Ingrese profundidad máxima Zmáx: "))
    res = 40
    print(f"Calculando esfuerzos verticales en plano vertical y={y_fijo}...")    
    x_range = np.linspace(ext_x1, ext_x2, res)
    z_range = np.linspace(0.1, z_max, res)
    X, Z = np.meshgrid(x_range, z_range)
    Sigma = np.array([[calcular_sigma_z(x_pts, y_pts, q_pts, xi, y_fijo, zi) for xi in x_range] for zi in z_range])

    plt.figure(figsize=(9, 7))
    cp = plt.contourf(X, Z, Sigma, levels=20, cmap='gist_rainbow_r')
    plt.colorbar(cp, label='Esfuerzo Vertical ($\\sigma_z$)')
    plt.gca().invert_yaxis()
  
    # Dibujar líneas de contorno
    lineas = plt.contour(X, Z, Sigma, levels=20, colors='black', linewidths=0.5)
    plt.clabel(lineas, inline=True, fontsize=10)    
    
    # Indicar la posición de la carga en superficie (z=0)
    # Mostramos el segmento del polígono que cruza el plano Y fijo
    plt.axhline(0, color='black', linewidth=2)
    plt.title(f"Perfil Vertical XZ (y = {y_fijo})")
    plt.xlabel("x"); plt.ylabel("Z (Profundidad)"); plt.grid(alpha=0.3); plt.show()

def graficar_perfil_yz(x_pts, y_pts, q_pts):
    x_fijo = float(input("Ingrese plano de corte X (ej. x=2): "))
    ext_y1 = float(input("Ingrese valor horizontal mínimo Ymín (ej. -10): "))
    ext_y2 = float(input("Ingrese valor horizontal mmáximo Ymáx (ej. 10): "))
    z_max = float(input("Ingrese profundidad máxima Zmáx: "))
    res = 40
    print(f"Calculando esfuerzos verticales en plano x={x_fijo}...")    
    y_range = np.linspace(ext_y1, ext_y2, res)
    z_range = np.linspace(0.1, z_max, res)
    Y, Z = np.meshgrid(y_range, z_range)
    Sigma = np.array([[calcular_sigma_z(x_pts, y_pts, q_pts, x_fijo, yi, zi) for yi in y_range] for zi in z_range])

    plt.figure(figsize=(9, 7))
    cp = plt.contourf(Y, Z, Sigma, levels=20, cmap='gist_rainbow_r')
    plt.colorbar(cp, label='Esfuerzo Vertical ($\\sigma_z$)')
    plt.gca().invert_yaxis()
  
    # Dibujar líneas de contorno
    lineas = plt.contour(Y, Z, Sigma, levels=20, colors='black', linewidths=0.5)
    plt.clabel(lineas, inline=True, fontsize=10)    
    
    # Indicar la posición de la carga en superficie (z=0)
    # Mostramos el segmento del polígono que cruza el plano Y fijo
    plt.axhline(0, color='black', linewidth=2)
    plt.title(f"Perfil Vertical YZ (x = {x_fijo})")
    plt.xlabel("y"); plt.ylabel("Z (Profundidad)"); plt.grid(alpha=0.3); plt.show()

def esfuerzo_vertical_P(x_pts, y_pts, q_pts):
    print("\n--- Configuración del Punto de Observación ---")
    x0 = float(input("Ingrese coordenada x0: "))
    y0 = float(input("Ingrese coordenada y0: "))
    z  = float(input("Ingrese profundidad z: "))
    resultado = calcular_sigma_z(x_pts, y_pts, q_pts, x0, y0, z)
    # Resultado del esfuerzo vertical en P
    print(f"Esfuerzo vertical (sigma_z) en ({x0}, {y0}, {z}) = {resultado:.6f}")
    
def main():
    print("\n--- POLÍGONO DE CARGA ---")
    nombre_archivo = input("Ingresa el nombre del archivo con extension:")
    # nombre_archivo = "Polig.L3Z.txt"
    
    if not os.path.exists(nombre_archivo):
        with open(nombre_archivo, "w") as f:
            f.write("# x y q\n0 0 100\n2 0 100\n2 2 100\n0 2 100\n0 0 0")
    # Lectura de datos del archivo    
    data = np.loadtxt(nombre_archivo)
    x_p, y_p, q_p = data[:, 0], data[:, 1], data[:, 2]
    
    #print(x_p)
    # Graficando el polígono de carga
    plt.figure(figsize=(9, 7))
    plt.plot(x_p, y_p, 'k--', linewidth=3, label='Área de Carga')
    plt.title("Polígono de carga")
    plt.xlabel("x"); plt.ylabel("y"); plt.legend(); plt.grid(alpha=0.3); plt.show()
   
    while True:
        print("\n--- MENÚ DE VISUALIZACIÓN ---")
        print("1. Valor del esfuerzo vertical en (X,Y,z)")
        print("2. Curvas de nivel en Planta (XY)")
        print("3. Bulbo de presiones en Perfil (XZ)")
        print("4. Bulbo de presiones en Perfil (YZ)")
        print("5. Salir")
        opcion = input("Seleccione una opción: ")

        if opcion == '1': esfuerzo_vertical_P(x_p, y_p, q_p)
        elif opcion == '2': graficar_planta_xy(x_p, y_p, q_p)
        elif opcion == '3': graficar_perfil_xz(x_p, y_p, q_p)
        elif opcion == '4': graficar_perfil_yz(x_p, y_p, q_p)
        elif opcion == '5': break
        else: print("Opción no válida.")

if __name__ == "__main__":
    main()
